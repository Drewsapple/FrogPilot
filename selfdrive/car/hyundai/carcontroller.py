...
class JerkLimiter:
  ...
  def make_jerk(self, CS, accel, actuators):
    ...
    if not self.hkg_tuning:
        self.jerk_upper_limit = 3.0 if actuators.longControlState == LongCtrlState.pid else 1.0
        self.jerk_lower_limit = 5.0
    else:
        startingJerk = 0.5
        jerkLimit = 5.0
  ...
  def calculate_limited_accel(self, accel, actuators, CS, LongCtrlState, interp, clip):
    ...
    limited_accel_delta = clip(accel_delta, -self.jerk_lower_limit * self.DT_CTRL, self.jerk_upper_limit * self.DT_CTRL)
